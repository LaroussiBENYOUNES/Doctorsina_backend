...
function init(botLogoPath) {
    ...
    template = ` <button class='chat-btn'><i  class="bi bi-chat-fill fx animated fadeInLeft" data-animate="fadeInLeft" width="30px"></i></button>

    <div class='chat-popup fx animated  pulse backdrop-blur'>

        <div class='chat-header'>
            <div class='chatbot-img'>
                <i  alt='Chat Bot image' class='bot-img bi bi-body-text'> </i>
            </div>
            <h3 class='bot-title'>DoctorSina Bot</h3>
            <button class = "expand-chat-window" ><i  class="material-icon bi bi-arrows-fullscreen" style="color:#778C7B;" width="30px"></i></button>
        </div>
    ...
    chatBtn.addEventListener("click", () => {
        ...
        if (chatPopup.style.display == "none" && mobileDevice) {
            ...
            chatInput.focus();
            chatBtn.innerHTML = `<i class="bi bi-x-lg fx animated fadeInLeft" data-animate="fadeInLeft" width="30px"></i>`
        } else if (mobileDevice) {
            chatPopup.style.display = "none"
            chatBtn.innerHTML = `<i  class="bi bi-chat-fill fx animated fadeInLeft" data-animate="fadeInLeft" data-animation-delay="50" width="30px"></i>`
        } else {
            mobileView()
        }
    })
    ...
    expandWindow.addEventListener("click", (e) => {
        ...
        if (expandWindow.innerHTML == '<img  class="material-icon bi bi-arrows-fullscreen" style="color:#778C7B;" width="30px"/>') {
            expandWindow.innerHTML = `<img  class="material-icon bi bi-arrows-fullscreen" style="color:#778C7B;" width="30px"></img>`
            root.style.setProperty('--chat-window-height', 80 + "%");
            root.style.setProperty('--chat-window-total-width', 85 + "%");
            chatHeader.style.width = "100%";
        } else if (expandWindow.innerHTML == '<img  class="material-icon bi bi-arrows-fullscreen" style="color:#778C7B;" width="30px"></img>') {
            chatPopup.style.display = "none"
